...
TEXTCOLORS = (None, "#003bd1", "#06ab00","#b5b52b","#f57f00","#703711","#de0000","#6a00b5","#000000")
...
class Grid:
        ...
        def setup(self, template, forbidden_place=[-1, -1]):
                """
                ...
                while not len(bombs) == template[2]: 
                        for y in range(template[1]):
                                for x in range(template[0]):
                                        ...
                                        if not [y, x] in bombs and not [y, x] == forbidden_place:
                ...
                for y in range(len(self.grid.grid)):
                        for x in range(len(self.grid.grid[y])):
                                if self.grid.grid[y][x][2] == 1:
                                        if self.grid.grid[y][x][1] == 0:
                                                t = "  "
                                        else:
                                                t = str(self.grid.grid[y][x][1])
                                        c = TEXTCOLORS[self.grid.grid[y][x][1]]
                                        self.btns[y][x][0].config(text=t, background="#c6c9c5", state=DISABLED, disabledforeground=c)
